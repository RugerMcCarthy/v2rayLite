...
import com.thoughtcrime.v2raylite.bean.V2rayConfig.Companion.DEFAULT_NETWORK
import com.thoughtcrime.v2raylite.bean.V2rayConfig.Companion.HTTP
        ...
        try {
            ...
            if (config.configType == EConfigType.CUSTOM) {
                ...
                val customConfig = if (raw.isNullOrBlank()) {
                 ...
                 Log.d(ANG_PACKAGE_LOG, customConfig)
                return Result(true, customConfig)
            }
            val outbound = config.getProxyOutbound() ?: return Result(false, "")
            val result = getV2rayNonCustomConfig(context, outbound)
            Log.d(ANG_PACKAGE_LOG, result.content)
            return result
        } catch (e: Exception) {
            e.printStackTrace()
            return Result(false, "")
